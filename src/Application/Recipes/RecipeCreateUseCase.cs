...
public class RecipeCreateUseCase
{
    ...
    public RecipeCreateUseCase(IUserRepository userRepo, IRecipeRepository recipeRepo)
    {
        _userRepo = userRepo;
    ...
    public async Task<Result<RecipeId>> CreateAsync(RecipeCreateDto dto)
    {
        ...
        if (!recipeTitleResult.IsSuccess)
        {
            return recipeTitleResult.Error!;
        }

        var recipeDescriptionResult = RecipeDescription.Create(dto.Description);
        if (!recipeDescriptionResult.IsSuccess)
        {
            return recipeDescriptionResult.Error!;
        }

        var recipeInstructionResult = RecipeInstruction.Create(dto.Instruction);
        if (!recipeInstructionResult.IsSuccess)
        {
            return recipeInstructionResult.Error!;
        }
        ...
        if (!isDifficultyDefined)
        {
            return RecipeErrors.DifficultyIsNotDefined;
        }

        var isParseSuccess = TimeSpan.TryParse(dto.CookingTime, out var cookingTime);
        if (!isParseSuccess)
        {
            return RecipeErrors.CookingTimeHasInvalidFormat;
        }

        if (cookingTime > TimeSpan.FromDays(7))
        {
            return RecipeErrors.CookingTimeIsTooHuge;
        }

        if (dto.Ingredients.Count == 0)
        {
            return RecipeErrors.NoIngredientsProvided;
        }

        var ingredientMappingResult = dto.Ingredients.Select(x => Ingredient.Create(x.Name, x.Count, x.UnitType)).ToList();
        var failedIngredient = ingredientMappingResult.FirstOrDefault(x => !x.IsSuccess);
        if (failedIngredient is not null)
        {
            return failedIngredient.Error!;
        }
